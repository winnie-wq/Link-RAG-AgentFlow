...
from spider import get_news  #导入爬虫函数
...
def get_hot_news():
    """
    获取热点新闻接口
    逻辑：先读Redis缓存 → 无缓存执行爬虫抓取 → 存入Redis缓存
    """
    cache_key = "news:hot_list"
    cache_data = get_cache(cache_key)
    if cache_data:
    ...
    # 缓存不存在，启动爬虫抓取
    news_data = get_news()
    # 抓取结果存入Redis，有效期1800秒（30分钟）
    set_cache(cache_key, news_data, 1800)
    return {"code": 200, "msg": "资讯实时抓取", "data": news_data}
